...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-body">
            ...
            <!-- ════════════════════ KEUANGAN SECTION ════════════════════ -->
            <div id="keuanganSection" style="display:none;">
                <div class="section-page-header reveal" style="background: linear-gradient(130deg, #164E63 0%, #0891B2 60%, #06B6D4 100%);">
                    <div class="section-page-header-text">
                        <div class="tag">✦ Keuangan</div>
                ...
                <div class="summary-strip reveal">
                    <div class="summary-chip">
                        <div class="summary-chip-icon" style="background:#ECFEFF;">💰</div>
                        <div class="summary-chip-text">
                            <div class="label">Total Tagihan</div>
                            <div class="value">{{ $totalInvoicesCount ?? 0 }}</div>
                        </div>
                    </div>
                    <div class="summary-chip">
                        <div class="summary-chip-icon" style="background:#ECFDF5;">✅</div>
                        <div class="summary-chip-text">
                            ...
                            <div class="value">{{ $paidInvoicesCount ?? 0 }}</div>
                        </div>
                    </div>
                    <div class="summary-chip">
                        <div class="summary-chip-icon" style="background:#FEF3C7;">⚠️</div>
                        <div class="summary-chip-text">
                            <div class="label">Sebagian</div>
                            <div class="value">{{ $partialInvoicesCount ?? 0 }}</div>
                        </div>
                    </div>
                    <div class="summary-chip">
                        <div class="summary-chip-icon" style="background:#FEF2F2;">❌</div>
                        <div class="summary-chip-text">
                            <div class="label">Belum Dibayar</div>
                            <div class="value">{{ $unpaidInvoicesCount ?? 0 }}</div>
                        </div>
                    </div>
                </div>
                    ...
                    <table>
                        ...
                        <tbody id="keuanganTableBody">
                            @if(isset($invoices))
                                ...
                                @php
                                    $statusColor = 'red';
                                    if ($invoice->status === 'lunas') $statusColor = 'green';
                                    elseif ($invoice->status === 'sebagian') $statusColor = 'amber';
                                @endphp
                                <tr>
                                    <td style="color:var(--text-muted);font-size:12px;font-family:'Space Mono',monospace;text-align:center;">{{ $index + 1 }}</td>
                                    <td>
                                        <div style="font-family:'Space Mono',monospace; font-size:12px; font-weight:700; color:var(--blue-deep); background:#F1F5F9; padding:4px 8px; border-radius:6px; display:inline-block;">{{ $invoice->invoice_number }}</div>
                                    </td>
                                    <td>
                                        <div style="font-size:13px; font-weight:500;">{{ $invoice->created_at->format('d M Y') }}</div>
                                        <div style="font-size:11px; color:var(--text-muted);">{{ $invoice->created_at->format('H:i') }}</div>
                                    </td>
                                    <td>
                                        <div style="font-weight:600; font-size:13px;">{{ optional($invoice->patient)->name ?? '-' }}</div>
                                        <div style="font-size:11px; color:var(--text-muted);">{{ optional($invoice->patient)->medical_record_number ?? '-' }}</div>
                                    </td>
                                    <td>
                                        @if(optional($invoice->admission)->admission_type === 'Rawat Inap')
                                            <span style="font-size:11px; font-weight:600; padding:4px 8px; border-radius:20px; background:#F5F3FF; color:#6D28D9;">Rawat Inap</span>
                                        @else
                                            <span style="font-size:11px; font-weight:600; padding:4px 8px; border-radius:20px; background:#FEF3C7; color:#92400E;">Rawat Jalan</span>
                                        @endif
                                    </td>
                                    <td style="font-weight:700; font-size:13px; color:var(--blue-deep);">Rp {{ number_format($invoice->amount, 0, ',', '.') }}</td>
                                    <td style="font-weight:600; font-size:13px; color:var(--text-muted);">Rp {{ number_format($invoice->paid_amount, 0, ',', '.') }}</td>
                                    <td>
                                        <div style="display:flex; align-items:center; gap:8px;">
                                            <div class="status-cell" style="color:var(--accent-{{ $statusColor }});">
                                                <span class="status-dot" style="background:var(--accent-{{ $statusColor }});"></span>{{ ucfirst($invoice->status) }}
                                            </div>
                                            @if($invoice->status !== 'lunas')
                                            <button type="button" class="btn-sm edit-keuangan" style="padding:4px 10px; font-size:11px; font-weight:600; border-radius:12px; background:var(--blue-bright); color:white; border:none; cursor:pointer;"
                                                data-id="{{ $invoice->id }}"
                                                data-invoice-number="{{ $invoice->invoice_number }}"
                                                data-patient="{{ optional($invoice->patient)->name }}"
                                    ...
                                    <td>
                                        <div class="act-wrap" style="justify-content:center;">
                                            <button type="button" class="act-btn view-keuangan" title="Lihat/Print"
                                                data-invoice-number="{{ $invoice->invoice_number }}"
                                                data-patient="{{ optional($invoice->patient)->name }}"
                                                data-admission-type="{{ optional($invoice->admission)->admission_type }}"
                                                data-total="{{ $invoice->amount }}"
                                                data-paid="{{ $invoice->paid_amount }}"
                                                data-status="{{ $invoice->status }}"
                                            ...
                                            <button type="button" class="act-btn edit edit-keuangan"
                                                ...
                                                data-paid="{{ $invoice->paid_amount }}"
                                                data-status="{{ $invoice->status }}"
                                                {{ $invoice->status === 'lunas' ? 'disabled style=opacity:0.5;' : '' }}
                                            >
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>
                                            </button>
                                            <button type="button" class="act-btn delete delete-keuangan" title="Hapus" data-id="{{ $invoice->id }}">
            ...
            <!-- Buat Tagihan Modal -->
            <div class="modal-overlay" id="buatTagihanModal">
                <div class="modal-window">
                    <button type="button" class="modal-close" id="closeBuatTagihanModalBtn">✕</button>
                    <h3>Buat Tagihan Baru</h3>
                    <p style="font-size:13px; color:var(--text-muted);">Pilih kunjungan pasien yang belum memiliki invoice.</p>

                    <form method="POST" action="{{ route('invoices.store') }}" id="buatTagihanForm">
                        @csrf
                        <div class="modal-grid">
                            <div class="modal-field full-width">
                                <label>Kunjungan</label>
                                <select name="admission_id" id="buatTagihan_admission_id" required>
                                    <option value="">-- Pilih Kunjungan --</option>
                                    @if(isset($admissionsWithoutInvoice))
                                        @foreach($admissionsWithoutInvoice as $adm)
                                            <option value="{{ $adm->id }}" data-patient-id="{{ $adm->patient_id }}" data-patient-name="{{ optional($adm->patient)->name }}">
                                                {{ $adm->registration_number }} - {{ optional($adm->patient)->name }} ({{ $adm->admission_type }})
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="modal-field full-width">
                                <label>Pasien</label>
                                <input type="text" id="buatTagihan_patient_name" disabled style="background:#F1F5F9;" placeholder="Pilih kunjungan untuk menampilkan pasien" />
                                <input type="hidden" name="patient_id" id="buatTagihan_patient_id">
                            </div>
                            <div class="modal-field full-width">
                                <label>Total Tagihan Otomatis (Rp)</label>
                                <div style="display:flex; gap:10px;">
                                    <input type="number" id="buatTagihan_amount" name="amount" required style="flex:1;" />
                                    <button type="button" id="hitungTagihanBtn" style="background:var(--blue-bright); color:white; border:none; padding:0 14px; border-radius:8px; cursor:pointer; font-size:13px; font-weight:600; min-width:80px;">Hitung</button>
                                </div>
                                <div style="font-size:11px; color:var(--text-muted); margin-top:4px;">Klik "Hitung" untuk menghitung otomatis, atau input manual.</div>
                            </div>
                            <div class="modal-field full-width">
                                <label>Catatan Tambahan</label>
                                <textarea name="notes" placeholder="Catatan tagihan..."></textarea>
                            </div>
                        </div>
                        <div class="modal-actions">
                            <button type="button" class="modal-button-secondary" id="cancelBuatTagihanModalBtn">Batal</button>
                            <button type="submit" class="button-primary">Simpan Tagihan</button>
                        </div>
                    </form>
                </div>
            </div>

            <!-- Keuangan Pay/Edit Modal -->
            <div class="modal-overlay" id="keuanganModal">
                <div class="modal-window">
                    <button type="button" class="modal-close" id="closeKeuanganModalBtn">✕</button>
                    <h3>Pembayaran Invoice</h3>
                    <p style="font-size:13px; color:var(--text-muted);">Pilih tipe pembayaran dan masukkan nominal.</p>

                    <form method="POST" action="" id="keuanganForm">
                        @csrf
                        <input type="hidden" name="_method" value="PUT">
                        <div class="modal-grid">
                            <div class="modal-field full-width">
                                <label>No. Invoice</label>
                                <input type="text" id="keuangan_invoice_number" disabled style="background:#F1F5F9;" />
                            </div>
                            <div class="modal-field full-width">
                                <label>Pasien</label>
                                <input type="text" id="keuangan_patient" disabled style="background:#F1F5F9;" />
                            </div>
                            <div class="modal-field">
                                <label>Total Tagihan (Rp)</label>
                                <input type="number" id="keuangan_total" disabled style="background:#F1F5F9;" />
                            </div>
                            <div class="modal-field">
                                <label>Sisa Tagihan (Rp)</label>
                                <input type="number" id="keuangan_sisa" disabled style="background:#F1F5F9;" />
                            </div>
                            <div class="modal-field full-width">
                                <label>Tipe Pembayaran</label>
                                <div style="display:flex; gap:16px; margin-top:4px;">
                                    <label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="radio" name="payment_type" value="sebagian" id="pay_sebagian" checked> Sebagian</label>
                                    <label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="radio" name="payment_type" value="lunas" id="pay_lunas"> Lunas</label>
                                </div>
                            </div>
                            <div class="modal-field full-width">
                        ...
                        <div class="modal-actions">
                            <button type="button" class="modal-button-secondary" id="cancelKeuanganModalBtn">Batal</button>
                            <button type="submit" class="button-primary">Proses Pembayaran</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="modal-overlay" id="invoiceViewModal">
                <div class="modal-window" style="max-width:600px; padding:0; overflow:hidden;">
                    <div style="padding:24px 24px 0 24px; display:flex; justify-content:space-between; align-items:center;">
                        <h3 style="margin:0;">Invoice / Tagihan</h3>
                        <button type="button" class="modal-close" id="closeInvoiceViewModalBtn" style="position:static;">✕</button>
                    </div>
                    
                    <div id="invoicePrintArea" style="padding:24px; background:#fff; margin:16px; border:1px solid #E2E8F0; border-radius:8px;">
                        <!-- Invoice Header -->
                        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:30px; border-bottom:1px solid #E2E8F0; padding-bottom:20px;">
                            <div>
                                <h1 style="margin:0; font-size:24px; color:var(--blue-deep);">MediCore Hospital</h1>
                                <p style="margin:4px 0 0; font-size:12px; color:var(--text-muted);">Jl. Kesehatan No. 123, Jakarta<br>Telp: (021) 123-4567</p>
                            </div>
                            <div style="text-align:right;">
                                <h2 style="margin:0; font-size:20px; color:#64748B;">INVOICE</h2>
                                <p id="view_inv_number" style="margin:4px 0 0; font-family:'Space Mono',monospace; font-weight:700; color:var(--blue-deep);">INV-0000</p>
                                <p id="view_inv_date" style="margin:4px 0 0; font-size:12px; color:var(--text-muted);">01 Jan 2026</p>
                            </div>
                        </div>
                        ...
                        <div style="display:flex; justify-content:space-between; margin-bottom:30px;">
                            <div>
                                <h4 style="margin:0 0 8px; font-size:12px; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.05em;">Tagihan Kepada:</h4>
                                <p id="view_inv_patient" style="margin:0; font-size:14px; font-weight:700; color:var(--blue-deep);">Nama Pasien</p>
                        ...
                        <div style="background:#F8FAFC; padding:16px; border-radius:8px;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:12px;">
                                <span style="font-size:14px; font-weight:600; color:#64748B;">Total Tagihan</span>
                                <span id="view_inv_total" style="font-size:16px; font-weight:700; color:var(--blue-deep);">Rp 0</span>
                            </div>
                            <div style="display:flex; justify-content:space-between; margin-bottom:12px; border-bottom:1px solid #E2E8F0; padding-bottom:12px;">
                                <span style="font-size:14px; font-weight:600; color:#64748B;">Sudah Dibayar</span>
                                <span id="view_inv_paid" style="font-size:16px; font-weight:700; color:var(--green-main);">Rp 0</span>
                            </div>
                            <div style="display:flex; justify-content:space-between;">
                                <span style="font-size:16px; font-weight:800; color:var(--blue-deep);">Sisa Tagihan</span>
                                <span id="view_inv_sisa" style="font-size:18px; font-weight:800; color:#E11D48;">Rp 0</span>
                            </div>
                        </div>
                    </div>

                    <div style="padding:16px 24px; background:#F8FAFC; display:flex; justify-content:flex-end; gap:12px; border-top:1px solid #E2E8F0;">
                        <button type="button" class="button-primary" id="printInvoiceBtn" style="display:flex; align-items:center; gap:6px;">
    ...
    <script>
        ...
        (function restoreActiveSection() {
            ...
            if (saved && saved !== 'dashboard') {
                ...
                if (item) {
                    ...
                    document.getElementById('keuanganSection').style.display = saved === 'keuangan' ? 'block' : 'none';
                }
            }
        })();
        ...
        document.querySelectorAll('.sidebar-item[data-section]').forEach(item => {
            item.addEventListener('click', function () {
                ...
                document.getElementById('keuanganSection').style.display = t === 'keuangan' ? 'block' : 'none';
        ...
        document.getElementById('viewAllOutpatient')?.addEventListener('click', function (e) {
            ...
            if (sectionItem) {
                ...
                document.getElementById('keuanganSection').style.display = 'none';
        ...
        const buatTagihanAdmissionSelect = document.getElementById('buatTagihan_admission_id');
        const hitungTagihanBtn = document.getElementById('hitungTagihanBtn');
        const buatTagihanAmount = document.getElementById('buatTagihan_amount');
        const buatTagihanPatientId = document.getElementById('buatTagihan_patient_id');

        document.getElementById('openBuatTagihanModalBtn')?.addEventListener('click', () => { buatTagihanModal.style.display='flex'; document.body.style.overflow='hidden'; });
        document.getElementById('closeBuatTagihanModalBtn')?.addEventListener('click', () => { buatTagihanModal.style.display='none'; document.body.style.overflow=''; });
        document.getElementById('cancelBuatTagihanModalBtn')?.addEventListener('click', () => { buatTagihanModal.style.display='none'; document.body.style.overflow=''; });
        buatTagihanModal?.addEventListener('click', e => { if (e.target === buatTagihanModal) { buatTagihanModal.style.display='none'; document.body.style.overflow=''; } });

        buatTagihanAdmissionSelect?.addEventListener('change', function() {
            const option = this.options[this.selectedIndex];
            buatTagihanPatientId.value = option.dataset.patientId || '';
            document.getElementById('buatTagihan_patient_name').value = option.dataset.patientName || '';
            buatTagihanAmount.value = ''; 
        });

        hitungTagihanBtn?.addEventListener('click', async () => {
            const admissionId = buatTagihanAdmissionSelect.value;
            if (!admissionId) {
                alert('Pilih kunjungan pasien terlebih dahulu!');
                return;
            }
            hitungTagihanBtn.textContent = '...';
            try {
                const response = await fetch(`{{ url('/api/admissions') }}/${admissionId}/finance`);
                if (response.ok) {
                    const data = await response.json();
                    buatTagihanAmount.value = data.total;
                } else {
            ...
            hitungTagihanBtn.textContent = 'Hitung';
        });

        // ── Keuangan Modal
        const keuanganModal = document.getElementById('keuanganModal');
        const keuanganForm = document.getElementById('keuanganForm');
        const keuanganNewPayment = document.getElementById('keuangan_new_payment');
        const radioSebagian = document.getElementById('pay_sebagian');
        const radioLunas = document.getElementById('pay_lunas');
        let currentSisa = 0;
        
        document.getElementById('closeKeuanganModalBtn')?.addEventListener('click', () => { keuanganModal.style.display='none'; document.body.style.overflow=''; });
        document.getElementById('cancelKeuanganModalBtn')?.addEventListener('click', () => { keuanganModal.style.display='none'; document.body.style.overflow=''; });
        keuanganModal?.addEventListener('click', e => { if (e.target === keuanganModal) { keuanganModal.style.display='none'; document.body.style.overflow=''; } });

        document.querySelectorAll('.edit-keuangan').forEach(btn => {
            btn.addEventListener('click', function () {
                keuanganForm.action = "{{ url('/invoices') }}/" + this.dataset.id;
                document.getElementById('keuangan_invoice_number').value = this.dataset.invoiceNumber || '';
                document.getElementById('keuangan_patient').value = this.dataset.patient || '';
                document.getElementById('keuangan_total').value = this.dataset.total || '0';
                
                const total = parseFloat(this.dataset.total) || 0;
                const paid = parseFloat(this.dataset.paid) || 0;
                currentSisa = total - paid;
                currentSisa = currentSisa > 0 ? currentSisa : 0;
                
                document.getElementById('keuangan_sisa').value = currentSisa;
                keuanganNewPayment.value = '';
                keuanganNewPayment.max = currentSisa;
                keuanganNewPayment.readOnly = false;
                if(radioSebagian) radioSebagian.checked = true;
        ...
        radioLunas?.addEventListener('change', () => {
            if (radioLunas.checked) {
                keuanganNewPayment.value = currentSisa;
                keuanganNewPayment.readOnly = true;
            }
        });
        radioSebagian?.addEventListener('change', () => {
            if (radioSebagian.checked) {
                keuanganNewPayment.value = '';
                keuanganNewPayment.readOnly = false;
            }
        });
        ...
        document.getElementById('closeInvoiceViewModalBtn')?.addEventListener('click', () => { invoiceViewModal.style.display='none'; document.body.style.overflow=''; });
        invoiceViewModal?.addEventListener('click', e => { if (e.target === invoiceViewModal) { invoiceViewModal.style.display='none'; document.body.style.overflow=''; } });

        document.querySelectorAll('.view-keuangan').forEach(btn => {
            btn.addEventListener('click', function () {
                document.getElementById('view_inv_number').textContent = this.dataset.invoiceNumber || '-';
                document.getElementById('view_inv_date').textContent = this.dataset.date || '-';
                ...
                const total = parseFloat(this.dataset.total) || 0;
                const paid = parseFloat(this.dataset.paid) || 0;
                const sisa = total - paid > 0 ? total - paid : 0;
                
                const formatRp = num => 'Rp ' + num.toLocaleString('id-ID');
                document.getElementById('view_inv_total').textContent = formatRp(total);
                document.getElementById('view_inv_paid').textContent = formatRp(paid);
                document.getElementById('view_inv_sisa').textContent = formatRp(sisa);
        ...
        document.querySelectorAll('.delete-keuangan').forEach(btn => {
            btn.addEventListener('click', function () {
                if (!confirm('Hapus data invoice ini?')) return;
                const f = document.createElement('form');
                f.method = 'POST'; f.action = `{{ url('/invoices') }}/${this.dataset.id}`;
                f.innerHTML = `<input type="hidden" name="_token" value="{{ csrf_token() }}"><input type="hidden" name="_method" value="DELETE">`;
                document.body.appendChild(f); f.submit();
            });
        });
